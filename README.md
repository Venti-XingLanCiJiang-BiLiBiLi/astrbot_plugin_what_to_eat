...
- **菜单管理**：可视化添加/删除菜品
- **会话切换**：支持按群聊/私聊切换管理不同菜单
- **ID 自动补全**：输入群号或用户 ID 时动态搜索已有菜单会话，最多匹配 6 条
- **随机推荐**：一键随机推荐菜品
- **开席推荐**：随机推荐多道菜品组成宴席
- **数据导入导出**：支持 JSON 格式的导入与导出
- **回复模板自定义**：可以修改机器人的回复格式

### 配置开关

在插件设置中可配置以下开关（保存后立即生效）：

| 开关 | 说明 |
|------|------|
| **初始化新会话菜单为默认菜单** | 开启后，新群/新用户首次触发插件时自动继承默认菜单的菜品。保存配置时自动填充所有空菜单。 |
| **默认菜单变更时同步至所有菜单** | 开启后，对默认菜单的添加操作自动同步至所有其他菜单。保存配置时自动执行一次全量同步。 |
...
├── menus/
│   ├── default.json          # 默认菜单
│   ├── group_123456.json     # 群聊菜单
│   └── private_789.json      # 私聊菜单
└── config.json               # 插件配置（含回复模板、开关等设置）
```

- 菜单按会话隔离，每个群/私聊有独立菜单文件
- 更新或重装插件不会丢失数据
